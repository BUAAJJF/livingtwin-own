...
#
# v6b: run_d455_v6.sh with one more rung on the ladder.  Measured 2026-09-01:
# fine-tuning at scale 0.5 climbed for its whole 1500 iterations (0.41 -> 2.50
# placements, still rising at the end) and the step straight from there to 1.0
# collapsed it inside 400 iterations -- grasp attempts 3.34 -> 0.13, mean
# reward 6.47 -> 0.93, the same shape as distilling blind.  The destination was
# not unreachable; the step was too big.  So 0.75 goes in between, and both
# remaining stages get more iterations because 1500 was not enough at 0.5
# either.
#
# A separate file rather than an edit, because bash reads a script lazily by
# byte offset and the wrist variant is running the original RIGHT NOW.
#
#   VARIANT=plain GPU=0 nohup setsid bash scripts/run_d455_v6b.sh \
...
case "$VARIANT" in
  plain)
    ...
    T_THREEQ=Mjlab-Pick-Place-PiperX-Vision-Robust-ThreeQ
  ...
  wrist)
    ...
    T_THREEQ=Mjlab-Pick-Place-PiperX-Vision-Robust-Wrist-ThreeQ
...
THREEQ_ITERS=${THREEQ_ITERS:-3500}
FULL_ITERS=${FULL_ITERS:-5500}
...
if [ -n "${SELFCHECK:-}" ]; then
  ...
  for t in "$T_DISTILL" "$T_HALF" "$T_THREEQ" "$T_FULL" "$T_EVAL_NOMINAL" "$T_EVAL_STRESS"; do
    say "task           $t"
  done
...
# --- 3. PPO at three quarters -----------------------------------------------

run_stage finetune_threeq \
  "$MM" run -n "$ENV_NAME" python -u scripts/finetune.py \
    --task "$T_THREEQ" --resume "$RH" --critic "$TEACHER" \
    --num-envs "$VISION_ENVS" --iterations "$THREEQ_ITERS" \
    --run-name "${TAG}_finetune_threeq" --device "cuda:$GPU" --seed 42 \
    --cadence object --logger tensorboard

THREEQ_RUN=$(run_dir_from_log "$OUT/finetune_threeq.log")
assert_ran_to_completion "$OUT/finetune_threeq.log"
RQ=$(latest_checkpoint "$THREEQ_RUN")
printf '%s\n' "$RQ" >"$OUT/finetune_threeq_checkpoint.txt"
say "three-quarter policy $RQ"

# Evaluated here, before the last rung.  If the full stage collapses the way
# the 0.5 -> 1.0 step did, this is the deliverable and it must have a number.
if ! stage_done eval_threeq; then
  eval_cell "$T_EVAL_STRESS" "$RQ" "${VARIANT}_threeq_on_stress" "$GPU" \
    >"$OUT/${VARIANT}_threeq_on_stress.log" 2>&1 || \
    fail "three-quarter evaluation failed"
  mark_done eval_threeq
fi

# --- 4. PPO at the measured dropout -----------------------------------------

run_stage finetune_full \
  "$MM" run -n "$ENV_NAME" python -u scripts/finetune.py \
    --task "$T_FULL" --resume "$RQ" --critic "$TEACHER" \
...
cat >"$OUT/manifest.json" <<JSON
{
  ...
  "half_checkpoint": "$RH",
  "threeq_checkpoint": "$RQ",
  "final_checkpoint": "$RF",
  "dropout_schedule": {"distill": 0.0, "half": 0.5, "threeq": 0.75, "full": 1.0},
  "iterations": {"distill": $DISTILL_ITERS, "half": $HALF_ITERS, "threeq": $THREEQ_ITERS, "full": $FULL_ITERS},
