...
- `src/piper_push/tasks/pick_place/cold_curriculum.py` + `cold_cfg.py`: the cold-start teacher tasks (`-Robust-Cold`, `-Robust-Cold-NoSight`): nominal DR and near-zero penalties at random init, stages opened by rolling capability gates (grasp attempts, placements, safe episodes) with persistence and no regression, ending at the full heavy profile.  `-Robust-Cold2[-NoSight]` (v10d, `APPROACH=1`) adds `mdp.approach_speed` (grasp-site speed above an allowance that falls from 0.6 m/s at 15 cm to 0.1 m/s at 3 cm), `mdp.object_disturbed` (object speed while not held) and `mdp.top_down_grasp` (verticality of the finger axis near/while holding; local +z of grasp_site is the approach axis, verified against finger geometry).  `scripts/run_v10c.sh` trains one from scratch, evaluates it on `-Robust` with three seeds and freezes a baseline only against the criteria declared in its `config_snapshot.json` (`scripts/v10c_verdict.py`).
