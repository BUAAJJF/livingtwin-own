...
def summarize_yaw_episodes(rows: Sequence[Mapping[str, Any]]) -> dict[str, float]:
    position = [float(row["final_position_error_m"]) for row in rows]
    yaw = [float(row["final_yaw_error_rad"]) for row in rows]
    return {
        ...
        "p90_final_position_error_m": _percentile(position, 90.0),
        "mean_final_yaw_error_rad": float(np.mean(yaw)),
        "median_final_yaw_error_rad": float(np.median(yaw)),
        "p90_final_yaw_error_rad": _percentile(yaw, 90.0),
...
def evaluate_yaw_policy(
    ...
    *,
    ...
    env = YawPlanarPushEnv(
        asset_path, env_config, seed=int(seeds[0]), physics_override=physics_override
    )
    model.eval()
    ...
    for seed in seeds:
        ...
        saturated = 0
        action_count = 0
        reward_sums = {"position": 0.0, "yaw": 0.0, "success": 0.0, "action": 0.0}
    ...
    return summarize_yaw_episodes(rows), rows
