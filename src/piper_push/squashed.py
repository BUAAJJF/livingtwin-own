...
Gaussians (tanh is a bijection).  ``entropy`` is ``H(u) + sum log(1 -
tanh^2(mu))``: the exact change-of-variables term evaluated at the mean
instead of in expectation, which is the standard differentiable proxy.
"""
...
class SquashedGaussianDistribution(Distribution):
  ...
  def entropy(self) -> torch.Tensor:
    n = self._normal
    return n.entropy().sum(dim=-1) + log1m_tanh2(n.mean).sum(dim=-1)  # type: ignore[union-attr]
